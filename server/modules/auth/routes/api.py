...
def api_login():
    ...
    # Si un hub ya registrado inicia sesión, actualizamos su estado de última conexión
    hub_id = (data.get("hub_id") or request.headers.get("X-Hub-Id") or "").strip()
    if hub_id:
        try:
            db.execute("UPDATE hubs SET last_seen = ?, online = 1 WHERE hub_id = ? AND user_id = ?", (_now(), hub_id, user["user_id"]))
        except Exception:
            pass
